...
class CoreDataManager {
    ...
    class func fetchObjById<T: NSManagedObject>(entityName:T.Type, id:Int64?) -> T?{
        if let id = id {
            let filtro = NSPredicate(format: "id = %d", id)
            let entity = CoreDataManager.fetchObj(entityName: T.self, predicate: filtro)
            return entity.first
        }else{
            return nil
        }
